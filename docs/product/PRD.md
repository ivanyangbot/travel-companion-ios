...
| Pin Gooey 视觉调参 Demo | 独立验证多 Pin 轮廓、贴边与数字成员融合样式 | 2...12 个可见 Pin、随机本地场景、各 Pin 二维位置、手动控制开关、模拟相机、滤镜/状态参数 | 随机单体/聚合样式、手动逐 Pin 拖动或首页式相机自动融合分离、地图平移缩放、首页同规则贴边/直角、共享 Gooey 轮廓、自适应宽度与任意 Pin 对稳定数字过渡 | 不接真实地图、POI、业务数字、网络或持久化 | 手动开关默认开启；关闭后 Pin 不可单独拖动且缩放可自动融合/分离；可见 Pin 数量与设置一致且聚合 Pin 只计一个；聚合对聚合数字连续合并；越界贴边与直角稳定；32pt 等高；Debug 构建通过 |
